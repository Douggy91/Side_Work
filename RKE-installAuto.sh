...
hostnamectl set-hostname RKE-SERVER
...
for (( i=0; i<${#workerArr[@]};i++)); do
	...
	let "v+=1"
	expect <<EOF 
	#spawn ssh ${workerArr[$i]} apt-get install -y expect
	#expect "connecting"
	#send "yes\r"
	#expect "password"
	#send "${password}\r"
...
for (( i=0; i<${#workerArr[@]};i++)); do
        ...
        let "v+=1"
        expect <<EOF
	spawn scp /root/.ssh/authorized_keys ${workerArr[$i]}:/root/.ssh/authorized_keys
	expect "password"
	send "${password}\r"
	expect eof
EOF
